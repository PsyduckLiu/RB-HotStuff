...
// GetCli asynchronously invokes a quorum call on configuration c
// and returns a AsyncCommand, which can be used to inspect the quorum call
// reply and error when available.
func (c *Configuration) GetCli(ctx context.Context, in *CliRequest) *AsyncCommand {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "clientpb.Client.GetCli",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
		r := make(map[uint32]*Command, len(replies))
		for k, v := range replies {
			r[k] = v.(*Command)
		}
		return c.qspec.GetCliQF(req.(*CliRequest), r)
	}

	fut := c.RawConfiguration.AsyncCall(ctx, cd)
	return &AsyncCommand{fut}
}
...
type QuorumSpec interface {
	...
	// GetCliQF is the quorum function for the GetCli
	// asynchronous quorum call method. The in parameter is the request object
	// supplied to the GetCli method at call time, and may or may not
	// be used by the quorum function. If the in parameter is not needed
	// you should implement your quorum function with '_ *CliRequest'.
	GetCliQF(in *CliRequest, replies map[uint32]*Command) (*Command, bool)
}
...
type Client interface {
	...
	GetCli(ctx gorums.ServerCtx, request *CliRequest) (response *Command, err error)
}
...
func RegisterClientServer(srv *gorums.Server, impl Client) {
	...
	srv.RegisterHandler("clientpb.Client.GetCli", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*CliRequest)
		defer ctx.Release()
		resp, err := impl.GetCli(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
}
...
// AsyncCommand is a async object for processing replies.
...
func (f *AsyncCommand) Get() (*Command, error) {
